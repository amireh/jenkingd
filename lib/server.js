...
function slackUrlToJenkins(url) {
  var fragments = decodeURIComponent(url).
    match(/[\/?&]text=([^&]+)/)[1].
    split(" ");

  var jobName = fragments[0];
  var buildId = fragments[1];

  return K.JENKINS_URL.replace(/\/$/, "") + "/job/" + jobName + "/" + buildId;
};
...
var routes = [
  ...
  { // slack retrigger command
    url: /^\/slack.*/,
    handler: function(request, respond, onError) {
      var jobLink = slackUrlToJenkins(request.url)

      jenkins.connect().then(function() {
        return jenkins.retrigger(jobLink);
      }).then(function() {
        respond(200, "Your job has been retriggered", textResponse);
      }, onError);
    }
  },

  { // slack job status command
    url: /^\/jobstatus.*/,
    handler: function(request, respond, onError) {
      var jobLink = slackUrlToJenkins(request.url)

      jenkins.getJob(jobLink).then(function(jobStatus) {
        respond(200, "Current job status: " + jobStatus.status, textResponse);
      }, onError);
    }
  },
...
service = server.listen(8777, function(request, response) {
  ...
  var url = request.url;
  var timeout;
  var respond = function(code, data, formatResp) {
    formatResp = formatResp || function(response, data) {
      response.setHeader('Content-Type', 'application/json; charset=UTF-8');
      return JSON.stringify(data || '{}');
    };

    var buffer = formatResp(response, data);
